...
//    if (got_recount==1) {log_offset=get_offset();}
    snprintf(sql_query,STR_MAX_SIZE,"UPDATE sys_trf SET offset=%llu where id=1",(long long int)log_offset);
#ifdef DEBUG
    printf(sql_query);
#endif
...
void exit_all()
{
		...
		snprintf(temp, MAXLEN, "%d hit/%d miss/%d total, time %d\n", cache_hit, cache_miss, cache_lines, (int)(stop_time1-start_time1));
		logmsg(temp);
#endif    
    ...
    if (unlink(PID_FILE)!=0) 
    	{
        snprintf(temp, STR_MAX_SIZE,"can't remove pid file: %s",PID_FILE);
        logmsg(temp);
        };
...
void sql_connect(MYSQL* lpmysql_link)
{
        ...
        if (lpmysql_link!=mysql_real_connect(lpmysql_link, DB_HOSTNAME, DB_USERNAME, DB_PASSWORD, DB_DATABASE, 3306, NULL, 0))
        {
            snprintf(temp, STR_MAX_SIZE,"can't connect to server, MySQL Error - %d: %s\n",mysql_errno(lpmysql_link), mysql_error(lpmysql_link));
            logcrt(temp);
            exit_all();
        }
    /* try to select DB */
    ...
    {
        /* log message about error */
        snprintf(temp, STR_MAX_SIZE,"Unable to select db, MySQL Error - %d: %s\n",mysql_errno(lpmysql_link), mysql_error(lpmysql_link));
        logcrt(temp);
        exit_all();
    }
...
off_t get_offset()
{
	...
	if (result==NULL) 
		{
		logcrt(_T("get_offset: can't get work offset from table sys_trf, shutting down."));
		exit_all();
		};
#ifdef DEBUG		
...
void sys_daemonize ()
{
    ...
    if (sys_logfile == -1) 
    {
        snprintf(temp, STR_MAX_SIZE,"can't open %s w",SESSION_LOG_FNAME);
        logcrt(temp);
        exit_all();
    }
...
int sql_exec(char* sql_filename)
{
	...
	{
		snprintf(temp, STR_MAX_SIZE,"sql_exec: Can't open %s for read.",sql_filename);
		logcrt(temp);
	        sig_shutdown(1);
	        return 1;
	}
	while(!feof(sqlscript))
	{
		...
		{
			/* log message about error */
			snprintf(temp,STR_MAX_SIZE,"sql_exec: %s:%d MySQL Error - %d: %s\n",sql_filename, count,mysql_errno(&mysql_link), mysql_error(&mysql_link));
			logcrt(temp);
			return 1;
		}
	}	
...
int squid_logrotate(void)
{
    ...
    if (squid_pid<2) {
    	logmsg(_T("monthly job: SquiD PID invalid. Silently skip logrotation..."));
    	result=1;
    }
    else {
    	struct stat finfo;
...
// buggy    	
    	...
    	if (-1==kill(squid_pid, SIGUSR1)) {
    		result=2;
    		snprintf(temp, STR_MAX_SIZE, "kill return error: %d (%s)", errno, "1");
    		logmsg(temp);
    		//sys_nerr
    	}
    	else {
    		while ((finfo.st_size!=0)||(!(finfo.st_size<logsize))){
    			sleep(1);
    			stat(squid_filename, &finfo);
    		}
    	}
...
printf ("\t-s: No offset usage. Just handle the logfile at all and quit.\n");
...
void squid_reconfig() /* config generation and squid reconfiguration routine */
{
//!МЮДН ОЕПЕДЕКЮРЭ Я СВЕРНЛ ЙНКНМЙХ ЯРЮРСЯ.
#ifdef IP_STAT
return;
#endif
...
{
	snprintf(temp,STR_MAX_SIZE,"reconfig: Can't open %s for read.",SQUID_ORIG_CONF);
	logcrt(temp);
	sig_shutdown(1);
	return;
}
...
{
	snprintf(temp,STR_MAX_SIZE,"Can't open %s for write, skipping reconfiguration.",SQUID_CONF);
	logerr(temp);
	return;
}
#ifdef DEBUG
...
*
*/
		if(mysql_query(&mysql,"select id, sysname, data from acl"))
		{
                /* log message about error */
			snprintf(temp,STR_MAX_SIZE,"logrotate: MySQL Error - %d: %s\n",mysql_errno(&mysql), mysql_error(&mysql));
			logcrt(temp);
			exit_all();
		}
		...
		for (int acl_current_record=0;acl_current_record<acl_rows;acl_current_record++)
		{
			...
			fputs(out_buffer, fw);
			snprintf(temp, STR_MAX_SIZE,"%s.%s",SQUID_CONF, acl_row[1]);
//			snprintf(out_buffer,STR_MAX_SIZE,"acl group_%s proxy_auth \"%s\"\n", acl_row[1], temp);
//			fputs(out_buffer, fw);

			if (NULL == (fg = fopen(temp, "w")))
			{
				snprintf(temp,STR_MAX_SIZE,"Can't open %s.%s for write, skipping reconfiguration.",SQUID_CONF, acl_row[0]);
				logerr(temp);
				return;
			}
			snprintf(sql_query,STR_MAX_SIZE,"select login from users where (used<quota or quota=0) and (status=0) and aid=%s",acl_row[0]);
			...
			for (current_record=0;current_record<rows_selected;current_record++)
			{
				row=mysql_fetch_row(res);
				if (row==NULL) 
				{
					logcrt(_T("squid_reconfig: no row"));
					exit_all();
				};
				snprintf(out_buffer,STR_MAX_SIZE,"%s\n", row[0]);
				fputs(out_buffer, fg);
...
*/
#ifdef DEBUG
printf("reconfig: configuration written, let's signal squid reload it\n");
#endif
squid_pid=squid_getpid();
if (squid_pid<2) 
{
	logmsg(_T("squid_reconfig: SquiD PID invalid. Silently skip reconfiguration..."));
}
else 
{
...
int sql_update_cmd_status(MYSQL* lmysql, int cmd_id, int status_id)
{
	time_t current_time;
	time(&current_time);
	snprintf(sql_query, STR_MAX_SIZE, "update queue set complete=%d, rtime=%d where id=%d", status_id, (int)current_time, cmd_id);
    if(mysql_query(lmysql,sql_query))
    {
        /* log message about error */
        snprintf(temp,STR_MAX_SIZE,"sql_update_cmd_status: MySQL Error - %d: %s\n",mysql_errno(lmysql), mysql_error(lmysql));
        logerr(temp);
        return 1;
    }
	if (0==mysql_affected_rows(lmysql))
	{
        snprintf(temp,STR_MAX_SIZE,"sql_update_cmd_status: id=%d, status=%d, query (%s)\n",cmd_id, status_id, sql_query);
        logerr(temp);
        return 1;
	}
return 0;	
...
#endif
        logrotated=timing->tm_year*100+timing->tm_mon;
		snprintf(temp, STR_MAX_SIZE, "insert delayed into queue (command, itime) values (3, %d)", (int)tmp);
        if (mysql_query(&mysql, temp))
        {
        /* log message about error */
        snprintf(temp,STR_MAX_SIZE,"time_check: MySQL Error - %d: %s\n",mysql_errno(&mysql), mysql_error(&mysql));
        logerr(temp);
        exit_all();
        }
        //squid_logrotate(1);
...
/* getting total users */
    ...
    {
        /* log message about error */
        snprintf(temp,STR_MAX_SIZE,"check_state_1: MySQL Error - %d: %s\n",mysql_errno(&mysql), mysql_error(&mysql));
        logerr(temp);
        exit_all();
    }
    res=mysql_store_result(&mysql);
    if (res==NULL) {logerr(_T("check_state_1: no result"));exit_all();};
    row=mysql_fetch_row(res);
    if (row==NULL) {logerr(_T("check_state_1: no row"));exit_all();};
...
/* getting trusted users */
    if(mysql_query(&mysql,"select count(*) from users where used<quota or quota=0"))
    {
        /* log message about error */
        snprintf(temp,STR_MAX_SIZE,"check_state_2: MySQL Error - %d: %s\n",mysql_errno(&mysql), mysql_error(&mysql));
        logerr(temp);
        exit_all();
    }
    mysql_query(&mysql, sql_query);
    
    if (NULL==(res=mysql_store_result(&mysql))) {logerr(_T("check_state_2: no result"));exit_all();};
    if (NULL==(row=mysql_fetch_row(res))) {logerr(_T("check_state_2: no row"));exit_all();};
...
#endif
/* restricted users */
    if(mysql_query(&mysql,"select count(*) from users where used>quota and quota<>0"))
    {
        /* log message about error */
        snprintf(temp,STR_MAX_SIZE,"check_state_3: MySQL Error - %d: %s\n",mysql_errno(&mysql), mysql_error(&mysql));
        logerr(temp);
        exit_all();
    }
//    res=mysql_store_result(&mysql);
    if (NULL==(res=mysql_store_result(&mysql))) {logerr(_T("check_state_3: no result"));exit_all();};
    if (NULL==(row=mysql_fetch_row(res))) {logerr(_T("check_state_3: no row"));exit_all();};
...
printf("after negative users \n");
    ...
    if (mysql_query(&mysql, "select id, command from queue where complete=0 limit 1"))
	{
		/* log message about error */
		snprintf(temp,STR_MAX_SIZE,"check_state_4: MySQL Error - %d: %s\n",mysql_errno(&mysql), mysql_error(&mysql));
		logerr(temp);
		exit_all();
	}
     ...
     */
     	...
     	switch (cmd) {
     		case 0:		cres=2;
     					break;
     		case 1:		cres=1;
     					if (1==sql_update_cmd_status(&mysql, cid, cres)) {logerr("queue command status update failed.");};
     					squid_reconfig();
     					cres=2;
     					break;
     		case 2:		cres=1;
     					if (1==sql_update_cmd_status(&mysql, cid, cres)) {logerr("queue command status update failed.");};
     					(1==squid_recount(&mysql))?cres=3:cres=2;
     					break;
     		case 3:		cres=1;
     					if (1==sql_update_cmd_status(&mysql, cid, cres)) {logerr("queue command status update failed.");};
...
//[9]-content type
    ...
    {
	out=s;
	// strsep ГЮЛЕМЕМЮ ЛЮЙПНЯНЛ
        for (field = fields; (*field = strsep(&out, " \t")) != NULL;)
        if (**field != '\0') if (++field >= &fields[10]) break;
        if (fields[0]!=NULL) point = strchr(fields[0], '.');
/* first parametr MUST contain ".", very simple test... */
        if (point!=NULL) *point = '\0';
	    else {errno=1;return false;};
	for (fieldnum=0;fieldnum<10;fieldnum++) if (fields[fieldnum]==NULL) return false;
/* advanced test, if enabled (default) - reduce perfomance */
#ifndef FAST
/* IP address must contain 3 dot's, test about one */
        if (strchr(fields[2],'.')==NULL) {errno=2;return false;}
/* code [3] must contain "/" */
        if (strchr(fields[3],'/')==NULL) {errno=3;return false;}
/* mode [8] must contain "/" */
        if (strchr(fields[8],'/')==NULL) {errno=4;return false;}
#endif /* FAST switch */
	point=NULL;
...
//                point++;
        ...
        if (atol(fields[4])==0) return true;
        if (strstr(fields[8],"NONE/-")!=NULL) return true;
        if (strstr(fields[3],"NONE/")!=NULL) return true;
#ifndef IP_STAT
        ...
        if (strstr(fields[3],"DENIED")!=NULL) return true;
//#ifndef CHEAT_MODE
        if (strstr(fields[3],"HIT")!=NULL) cached=true;
//#endif /* cheat */
#ifdef DEBUG
...
static int uidcache;
		...
		static int uid;
#ifndef IP_STAT		
		login_size=mysql_real_escape_string(&mysql, login, fields[7], strlen(fields[7]));
#else
		login_size=mysql_real_escape_string(&mysql, login, fields[2], strlen(fields[2]));
#endif

#ifdef DEBUG
		printf("field %s login(%d) %s \n",fields[7], login_size, login);
#endif
	...
	snprintf(sql_query, STR_MAX_SIZE,"select id from users where login='%s';", login);
	if (mysql_query(&mysql,sql_query))
	{
	/* log message about error */
                snprintf(temp, STR_MAX_SIZE, "parser: mysql error, user: %s query: %s error %s", login, sql_query,mysql_error(&mysql));
                logmsg(temp);
                errno=5;
	...
	{
                if (NULL!=result) mysql_free_result(result);
				if (1==autolearn) 
				{
					snprintf(sql_query, STR_MAX_SIZE, "insert delayed into users (aid, quota, dquota, login, descr) select o1.value as aid, o2.value as lim1 ,o2.value as lim2, '%s', 'autocreated' from options as o1, options as o2  where o1.name='def_timeacl' and o2.name='std_limit'", login);
					if (mysql_query(&mysql,sql_query))
					{
					/* log message about error */
								snprintf(temp, STR_MAX_SIZE, "parser: autoadd fail, user: %s, query: %s, error: %s", login, sql_query,mysql_error(&mysql));
								logmsg(temp);
								errno=5;
								return false; //no user found
					}
				...
				{
                snprintf(temp, STR_MAX_SIZE, "parser: no user: %s query: %s error %s", login, sql_query,mysql_error(&mysql));
                logmsg(temp);
				errno=6;
				return false;
				}
	...
	{
		ucpointer++;
		if (USER_CACHE_SIZE==ucpointer) ucpointer=0;
	};
	if ((USER_CACHE_SIZE-1)>uccsize) {uccsize++;};
    }
    cache_lines++;
    {
#ifndef CHEAT_MODE        	
...
#endif /* cheat */	
	{
            snprintf(sql_query, STR_MAX_SIZE,"update users set used=used+%s where id=%d;",fields[4], uid);
#ifdef DEBUG
		...
		snprintf(temp, STR_MAX_SIZE, "no user found: %d query: %s\n", uid, sql_query);
                logerr(temp);
                errno=6;
                return false;
            };
            site_size=mysql_real_escape_string(&mysql, site, host, strlen(host));            
            snprintf(sql_query,STR_MAX_SIZE,"update site set size=size+%s, lutime=%s where u_id=%d and site='%s';",fields[4], fields[0] ,uid, site);
#ifdef DEBUG
            printf("parser: affected=%d, updating site %s\n",affect,sql_query);
#endif  /* DEBUG */
            mysql_query(&mysql, sql_query);
            affect=mysql_affected_rows(&mysql);
            if (0 == affect)
            { /* new host, add to table */
                snprintf(sql_query,STR_MAX_SIZE,"insert delayed into site (size, u_id, site, lutime, futime) values ( %s, %d,'%s', %s, %s );",fields[4], uid, site, fields[0], fields[0]);
#ifdef DEBUG
                printf("parser: affected=%d, inserting into site, %s\n", affect, sql_query);
#endif  /* DEBUG */
                mysql_query(&mysql, sql_query);
            };
#ifdef DEBUG
            if (affect>1)
            {
                ...
                logmsg(_T("parser: warning, more than one items affected in table site\n"));
                logmsg(sql_query);
            };
#endif /* DEBUG */
        }
        if (1==detailed)
        {
        	url_size=mysql_real_escape_string(&mysql, url, fields[6], strlen(fields[6]));
        	ip=ntohl(inet_addr(fields[2]));
            snprintf(sql_query,STR_MAX_SIZE,"insert delayed into detail (utime, qtime, ip_addr, code, size, method, url, u_id, server) values (%s,%s,%lu,'%s',%s,'%s','%s',%d,'%s')",fields[0],fields[1],ip,fields[3],fields[4],fields[5],url,uid,fields[8]);
#ifdef DEBUG
            printf("parser: inserting into detail: %s\n",sql_query);
#endif  /* DEBUG */
                mysql_query(&mysql, sql_query);
#ifdef DEBUG
                if (0==mysql_affected_rows(&mysql)) {logerr(_T("can't insert detailed record, look for db permissions!\n"));};
#endif  /* DEBUG */
        };
        };
    };
...
#endif 
for (int i=0; i<USER_CACHE_SIZE;i++)	ucache[i].uname = _V2PC(malloc(MAXBUFSIZE));
...
if (squid_filename == NULL)	squid_filename = SQUID_LOGNAME;  /* default filename */
/* Checking for runned program */
if(checkRunningPid()>0) exit(1);
...
#endif  /* DEBUG */
            logerr(squid_filename);logerr(_T("Can't get access for read logfile")); exit_all();break;
        }
...
//		}
#ifdef DEBUG
                snprintf(temp,STR_MAX_SIZE,"Working with offset: %ld",offset);
                logmsg(temp);
		...
		while(!feof(fp)&&(cycle_counter<USER_TEST_DELAY*10))
		{
...
/* send message about error to syslog */
		switch (errno)
		{
			case 1:	snprintf(temp, STR_MAX_SIZE,"parser: string incorrect, not enought fields in line:\n");
				break;
			case 2:	snprintf(temp, STR_MAX_SIZE, "parser: string incorrect, can't find '.' in timestamp. line:\n");
				break;
			case 3:	snprintf(temp, STR_MAX_SIZE, "parser: string incorrect, can't find '/' in code. line:\n");
				break;
			case 4:	snprintf(temp, STR_MAX_SIZE, "parser: string incorrect, can't find '/' in mode. line:\n");
				break;
		} /* switch */
					logmsg(temp);
#ifdef DEBUG
					fseeko(fp,squid_loffset,SEEK_SET);
...
//		fclose(fp);
		...
		snprintf(temp,STR_MAX_SIZE,"\n\nNew offset is - %llu, it takes %d seconds.",offset,stop_time-start_time);
		logmsg(temp);
